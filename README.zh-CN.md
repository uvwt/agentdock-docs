...
该命令会执行 TypeScript 检查和语言偏好测试，校验英文与简体中文文档的路径和结构一致，并分别完成两种语言的生产构建。

## 文档结构

```text
docs/                                            # 英文源文档
├── getting-started/                             # 安装与首次连接
...
- 首次访问时根据浏览器语言自动选择英文或简体中文；用户通过语言菜单手动选择后，将偏好写入 `localStorage`，后续优先使用手动选择。
- 共用图片保留在 `static/img/`，不重复存放与语言无关的资源。

## 内容规范

- 用户页面先给出可完成的最短路径，内部实现、完整参数和维护者流程进入参考、运维或开发者章节。
