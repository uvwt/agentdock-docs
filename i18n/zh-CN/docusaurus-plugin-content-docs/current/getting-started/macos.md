...
普通用户推荐使用 macOS 图形应用。整个过程不需要终端、Go、Git 或源码。
...
Apple 芯片和 Intel Mac 使用同一个 DMG，不需要判断处理器型号，也不需要下载 ZIP。

## 2. 第一次打开

当前免费版本还没有经过 Apple 公证，因此第一次启动需要手动确认一次：
...
MCP 客户端也在这台 Mac 上时，选择“仅本机”。这是最简单、最安全的首次安装方式，不需要域名或 Cloudflare 账号。

### 临时公网地址

需要从 ChatGPT、手机或其他设备连接，但还没有域名时，选择“临时地址”。AgentDock 会自动生成一个 HTTPS 公网地址。

这个地址可能在 Mac 或 Tunnel 重启后变化。地址变化时，从控制面板复制新地址，并替换客户端中的旧地址。

### 固定域名

准备长期使用稳定地址时，选择“固定域名”，并填写：
...
控制面板还可以测试公网地址、切换连接方式和重新生成临时地址。重新生成期间会隐藏旧地址，取得新地址后再显示。
...
普通用户建议两个都保持开启。也可以只关闭菜单栏开关，让核心服务继续在后台自动运行。
